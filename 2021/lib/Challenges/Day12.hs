...
module Challenges.Day12 (day12) where
import Data.Array
import Utils
...
solve _ (_, _, _) _ 1 = 1 -- index 1 is end
...
part1 (con, sz) = solve tsfm (con, sz, accumArray (+) 0 (bounds con) []) True 0 -- index 0 is start
    where tsfm nsz ps _ = map (True,) . filter (\e -> nsz!e == Big || ps!e < 1)

part2 :: (Map [Int], Map Size) -> Int
part2 (con, sz) = solve tsfm (con, sz, accumArray (+) 0 (bounds con) []) False 0 where
    tsfm nsz ps True = map (True,) . filter (\e -> nsz!e == Big || ps!e < 1)
    tsfm nsz ps False = map (\e -> (not (nsz!e == Big || ps!e < 1), e)) . filter (/=0)
