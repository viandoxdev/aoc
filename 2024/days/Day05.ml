...
module P = struct
  ...
  let rule = both digits (char '|' *> digits)
  let update = sep_by (char ',') digits
  let rules = many (rule <* char '\n')
  let updates = many (update <* char '\n')
...
let rule_array rules =
  ...
  List.iter (fun (a, b) -> arr.(a) <- b :: arr.(a)) rules;
  arr
...
let middle_page_sum updates = List.map middle_page updates |> sum
...
let day05 input =
  ...
  ( string_of_int @@ middle_page_sum ordered,
    string_of_int @@ middle_page_sum reordered )
