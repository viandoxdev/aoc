...
let rec head_nth n xs =
  if n > 0 then 
    List.hd xs :: head_nth (n - 1) (List.tl xs)
  else []

let rec sliding_window s xs =
  if List.length xs <= s then [xs]
  else
    (head_nth s xs) :: sliding_window s (List.tl xs)

let sliding_pairs xs =
  let (_, pairs) = List.fold_left (fun (last, acc) c -> c, (last, c) :: acc) (List.hd xs, []) (List.tl xs) in
  List.rev pairs
